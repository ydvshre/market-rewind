...
app.get("/api/health", (_, res) =>
  res.json({
    ok: true,
    mode: process.env.ALPHA_VANTAGE_KEY
      ? "live-or-fallback"
      : "demo",
  })
);
...
app.get("/api/watchlist", authRequired, async (req, res) => {
  try {
    const oldCheckpoint = db
      .prepare(
        "SELECT last_viewed_at FROM user_checkpoints WHERE user_id=?"
      )
      .get(req.user.id);

    const checkpoint =
      oldCheckpoint?.last_viewed_at ||
      new Date(0).toISOString();

    const items = db
      .prepare(
        "SELECT ticker FROM watchlist_items WHERE user_id=? ORDER BY added_at DESC"
      )
      .all(req.user.id);

    const tickers = items.map((x) => x.ticker);

    // Shared market computation.
    // It does not use req.user.id.
    await refreshTickers(tickers);

    const now = new Date().toISOString();

    db.prepare(`
      INSERT INTO user_checkpoints(user_id,last_viewed_at)
      VALUES(?,?)
      ON CONFLICT(user_id)
    ...
    res.json({
      ...
      events,
      mode: process.env.ALPHA_VANTAGE_KEY
        ? "Live when API succeeds; otherwise demo fallback"
        : "Demo mode",
    });
  } catch (e) {
    console.error(e);
...
app.post("/api/watchlist", authRequired, async (req, res) => {
  try {
    ...
    db.prepare(`
      INSERT OR IGNORE INTO watchlist_items(user_id,ticker,added_at)
      VALUES(?,?,?)
    ...
    res.status(400).json({
      error: "Could not add that ticker.",
    });
  }
});
...
// Useful for the demo:
// creates another shared market observation
// without changing a user's checkpoint.
app.post("/api/market/refresh", authRequired, async (req, res) => {
  try {
    const items = db
      .prepare(
        "SELECT ticker FROM watchlist_items WHERE user_id=?"
      )
      .all(req.user.id);

    const stocks = await refreshTickers(
      items.map((x) => x.ticker)
    );
...
app.listen(PORT, "0.0.0.0", () => {
  console.log(
    `Market Rewind running on port ${PORT}`
  );
});